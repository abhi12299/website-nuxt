...
const getMatchedText = (phrase, searchText) => {
  const MAX_CHARS = 100
  const phraseIndex = searchText.search(new RegExp(phrase, 'ig'))
  if (phraseIndex < 0) {
    // just return first 100 chars of the searchText
    return `${searchText.substr(0, MAX_CHARS)}...`
  }
  const phraseInText = searchText.substr(phraseIndex, phrase.length)
  ...
  if (phraseIndex + phrase.length < MAX_CHARS) {
    ...
    return `${searchText
      .substr(0, MAX_CHARS)
      .replace(phraseInText, `<b>${phraseInText}</b>`)}...`
  }
  const phraseLength = phraseInText.length
  const firstSubstr = (MAX_CHARS - phraseLength) / 2
  return `${searchText.substr(0, firstSubstr)}... ${searchText
    .substr(phraseIndex - 15, phrase.length + 15)
    .replace(phraseInText, `<b>${phraseInText}</b>`)}...`
}
