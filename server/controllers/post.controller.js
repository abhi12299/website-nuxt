...
postRouter.get('/getPost', async (req, res) => {
  ...
  if (error) {
    logger.error('Query params validation fail on /getPost:', {
      error,
      query: req.query
    })
    return res
...
postRouter.get('/getLatestPosts', async (req, res) => {
  ...
  if (error) {
    logger.error('Query params validation fail on /getLatestPosts', {
      error,
      query: req.query
    })
    return res
...
postRouter.get('/getAllPosts', async (req, res) => {
  ...
  if (error) {
    logger.error('Query params validation fail on /getAllPosts', {
      error,
      query: req.query
    })
    return res
  ...
  const { keywords } = req.query // match metaKeywords
